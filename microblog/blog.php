<?php	
	...
	function outputCategoryPosts(){
		...
		foreach (DB::$obj as &$entry) {
			$categoryExists = isset(DB::$obj["categories"][$_GET["category"]]);
			if(isset($entry[ID]) && ( $categoryExists && in_array($entry[ID],DB::$obj["categories"][$_GET["category"]]) || $_GET["category"]==="all") ){
				$values["id"]=$entry[ID];
